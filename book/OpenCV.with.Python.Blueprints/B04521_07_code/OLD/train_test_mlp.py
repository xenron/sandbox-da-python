...
def main():
    ...
    (X_train,y_train),(X_test,y_test),_,_ = homebrew.load_data("datasets/faces_indoor_training.pkl", num_components=50,
        test_split=0.2, save_to_file="datasets/faces_preprocessed.pkl", seed=42)
    ...
    params = dict( term_crit = (cv3.TERM_CRITERIA_COUNT, 300, 0.01),
        train_method = cv3.ANN_MLP_TRAIN_PARAMS_BACKPROP, bp_dw_scale = 0.001,
        bp_moment_scale = 0.9 )
    ...
    for l1 in xrange(10):
        ...
        layerSizes = np.int32([numFeatures, (l1+1)*numFeatures/5, numClasses])
