...
def save_image(im, i):
    ...
    Image.fromarray(a).save('.\\HandWritten\\%d.png' % i)


if __name__ == "__main__":
    path = u'..\\算法讲师\\机器学习升级版\\数据\\'
    print 'Load Training File Start...'
    data = np.loadtxt(path + '10.optdigits.tra', dtype=np.float, delimiter=',')
    x, y = np.split(data, (-1, ), axis=1)
    images = x.reshape(-1, 8, 8)
    y = y.ravel().astype(np.int)

    print 'Load Test Data Start...'
    data = np.loadtxt(path + '10.optdigits.tes', dtype=np.float, delimiter=',')
