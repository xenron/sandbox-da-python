...
def save_parameter(pi, A, B, time):
    f_pi = open(".\\text\\pi%d.txt" % time, "w")
    list_write(f_pi, pi)
    f_pi.close()
    f_A = open(".\\text\\A%d.txt" % time, "w")
    for a in A:
        list_write(f_A, a)
    f_A.close()
    f_B = open(".\\text\\B%d.txt" % time, "w")
    for b in B:
        list_write(f_B, b)
    f_B.close()
...
def load_train():
    f = file(".\\text\\pi.txt", mode="r")
    for line in f:
        pi = map(float, line.split(' ')[:-1])
    f.close()

    f = file(".\\text\\A.txt", mode="r")
